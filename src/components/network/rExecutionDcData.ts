function generateData() {
  ...
  let data: { dcName: string; dcType: string; location: string; measure: string }[] = []
        ...
        measures.forEach((measure) => {
          ...
          days.forEach((day) => {
            ...
            if (measure === 'On time delivery' || measure === 'Lead time') {
              dailyValues = { ...dailyValues, [day]: (Math.random() * 10).toFixed(2) } // Units are in days or percentage
            } else if (measure === 'Cost per shipment') {
              dailyValues = { ...dailyValues, [day]: (Math.random() * 200 + 100).toFixed(2) } // Cost in USD
            } else {
              dailyValues = { ...dailyValues, [day]: Math.floor(Math.random() * 1000) + 100 } // Volumes, counts
            }
          })
