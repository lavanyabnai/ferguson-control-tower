...
const numberToColor = (params: object) => {
  ...
  if (
    (params as any).data?.measure === 'Target Plan (Revenue)' ||
    (params as any).data?.measure === 'Financial Forecast (Revenue)' ||
    (params as any).data?.measure === 'POS (Revenue)' ||
    (params as any).data?.measure === 'Weighted Sales Price'
  ) {
    return ''
  }
  if ((params as any).data?.measure === 'Achievement Rate') {
    if ((params as any).value < 75) {
      return '#ffaaaa'
    }
    if ((params as any).value < 90) {
      return '#fef9c3'
    }
    return '#aaffaa'
  }
