function generateData() {
  ...
  let data: { storeName: string; storeType: string; location: string; measure: string }[] = []
        ...
        measures.forEach((measure) => {
          ...
          days.forEach((day) => {
            ...
            if (measure === 'Daily Sales (Revenue)') {
              dailyValues = { ...dailyValues, [day]: (Math.random() * 10000 + 500).toFixed(2) } // Revenue in USD
            } else if (measure === 'Customer Foot Traffic') {
              dailyValues = { ...dailyValues, [day]: Math.floor(Math.random() * 500 + 50) } // Number of customers
            } else if (measure === 'Staff Working Hours') {
              dailyValues = { ...dailyValues, [day]: Math.floor(Math.random() * 80 + 160) } // Total hours
            } else if (measure === 'Promotional Activity Effectiveness') {
              dailyValues = { ...dailyValues, [day]: (Math.random() * 100).toFixed(2) } // Effectiveness in percentage
            } else {
              dailyValues = { ...dailyValues, [day]: Math.floor(Math.random() * 300 + 50) } // General units or levels
            }
          })
