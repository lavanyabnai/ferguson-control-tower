...
import { generatedAccuracyData } from '@/components/lowes/rforecastAccuracy'
...
function numberToColor(params: CellClassParams<any, any>) {
  if (params.data.measure === 'Target Plan (Revenue)' || 
  ...
  if (params.data.measure === 'Achievement Rate') {
    if (params.value < 75) {
      return '#ffaaaa'  // Red for low achievement
    }
    if (params.value < 90) {
      return '#fef9c3'  // Yellow for medium achievement
    }
    return '#aaffaa'    // Green for high achievement
  }
  return ''
}
...
export default function ForecastDemand() {
  ...
  const rowData = generatedAccuracyData;
